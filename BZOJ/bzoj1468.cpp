...
struct Dfz{
	...
	int cal(int x,int D=0){
		int ans=0;d[x]=D;b[0]=0;dfs(x);
		sort(b+1,b+b[0]+1);
		for(int l=1,r=b[0];l<r;b[l]+b[r]<=k?ans+=r-l,l++:r--);
		return ans; 
	}
	int ans;
	int main(int x){
	...
	int main(){son[ans=0]=inf;k=gi;groot(1);main(1);return ans;}
}D;
int n;
int main(){
	n=gi;
	for(int i=1,u,v,w;i<n;i++)u=gi,v=gi,w=gi,D.G.lnk(u,v,w);
	D.resize(n);printf("%d\n",D.main());
	return 0;
}
