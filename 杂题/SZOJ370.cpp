...
const int N = (1 << 13), P = 1000000007, MAXN = (N << 1) + 10;
...
int main(){
	int i, c;
	n = inp();  init();
	for(i = 0; i < n; i++) a[i] = inp(); c = a[0];
	for(fac[0] = i = 1; i <= n; i++) fac[i] = mul(fac[i - 1], i);
	for(rfac[n] = fexp(fac[n], P - 2), i = n; i; i--) rfac[i - 1] = mul(rfac[i], i);
	for(i = 0; i < n; i++) g[i] = rfac[i + 1];
	inv(n, g, f);
	for(i = 0; i < n; i++) f[i] = mul(f[i], fac[i]);
	for(i = 1; i < n; i += 2) f[i] = P - f[i];
	for(i = 0; i < n; i++) a[i] = mul(a[i], fac[i]);
	for(i = 0; i < n; i++) g[i] = mul(f[n - 1 - i],rfac[n - 1 - i]);
	conv(n, a, n, g, n + n, h);
	for(printf("%d ", c), i = 1; i <= n; i++) 
		printf("%d ", mul(h[n + i - 2], rfac[i]));
	return 0;
}
